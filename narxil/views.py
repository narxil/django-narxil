...
def bookmark_list(request):
    ...
    return render(request, 'narxil/bookmark_list.html', context)
...
def bookmark_user(request, username):
    ...
    return render(request, 'narxil/bookmark_user.html', context)
...
def bookmark_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            bookmark.save()
            form.save_m2m()
            return redirect('narxil_bookmark_user',
                username=request.user.username)
    else:
        form = BookmarkForm()
    context = {'form': form, 'create': True}
    return render(request, 'narxil/form.html', context)
